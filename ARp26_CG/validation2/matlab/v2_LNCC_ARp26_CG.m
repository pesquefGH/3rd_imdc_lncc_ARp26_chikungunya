% Validation 2: LNCC ARP26 Chikungunya forecast model (high-order AR(p) model)
...
% (from EW 01 of 2021 to EW 25 of 2023). Model order p = floor(length(data)/2)-1. 
...
ind_v2=441+52;  % index of the end of EW 25 of 2023 (upper bound 
% observed data for validation 2)

cc=kcc(ind_01_21:ind_v2);  % observed cases from EW 1 of 2021 to EW 25 of 2023

cc(cc==0)=0.1;  % replaces null value in cc with a small value. 
% This is because we will apply 
% a log2 function to the data values. 

lwea=79;   % number of EW to be forecast, i.e., from EW 26 of 2023 
% up to EW 52 2024 (will be cropped later to EW 40 of 2024)
...
date=S_dates(719:770);  % Sunday dates (EW) to be forecast 
...
writetable(T,['..\spreadsheets\v2_ARp26_CG_',num2str(state_code(1)),'.csv'],'Delimiter',',')
...
print(['..\plots\v2_ARp26_CG_',UF],'-dpdf')
