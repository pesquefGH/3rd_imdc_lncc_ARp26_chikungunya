% Validation 1: LNCC ARP26 Chikungunya forecast model (high-order AR(p) model)
...
% (from EW 01 of 2021 to EW 25 of 2022). Model order p = floor(length(data)/2)-1. 
...
ind_v1=441;  % index of the end of EW 25 of 2022 (upper bound 
% observed data for validation 1)

cc=kcc(ind_01_21:ind_v1);  % observed cases from EW 1 of 2021 to EW 25 of 2022

cc(cc==0)=0.1;  % replaces null value in cc with a small value. 
% This is because we will apply 
% a log2 function to the data values. 

lwea=79;   % number of EW to be forecast, i.e., from EW 26 of 2022 
% up to EW 52 2023 (will be cropped later to EW 40 of 2023)
...
indf_ini=457; % time index of the EW 41 2022
indf_end=508; % time index of the EW 40 2023

% Writes a CSV file with the known and forecast data

date=S_dates(667:718);  % Sunday dates (EW) to be forecast 
...
writetable(T,['..\spreadsheets\v1_ARp26_CG_',num2str(state_code(1)),'.csv'],'Delimiter',',')
...
cases=kcc(EW_index);  % known observed chikungunya cases
...
print(['..\plots\v1_ARp26_CG_',UF],'-dpdf')
